...
int main()
{
	...
	*/
	...
	//use find to find things
	//find returns an iterator
	//if it does NOT find the item, map.end()
	std::string itemToFind = "Fries";
	auto wasItemFound = menu.find(itemToFind);
	if (wasItemFound == menu.end())//not found
		std::cout << "We do not have " << itemToFind << " on the menu. Try McDonald's.\n";
	else
	{
		std::cout << itemToFind << " costs " << wasItemFound->second << "\n";
		//2 ways to update the value...
		//1) (good way) use the iterator
		wasItemFound->second *= 1.25F;
		std::cout << "Now it costs " << wasItemFound->second << "!! Thanks Putin!\n";
		//2) (not as good) use map[key] = value;
		//menu[itemToFind] *= 1.25;
	}
...
"Isaac","Nguyen","Braulio","Anthony","Samuel","Victor"
	...
	while (true)//ctrl-K,S (surrounds with)
	{
		PrintGrades(grades);
		...
		if (foundStudent == grades.end())
			std::cout << studentName << " is not in PG2.\n";
		else
		{
			foundStudent->second = std::min(100.0, foundStudent->second + 5);
			std::cout << studentName << " now has a grade of " << foundStudent->second << "\n";
		}
	}
